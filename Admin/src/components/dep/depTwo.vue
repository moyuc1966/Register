<template>
    <div class="main">
        ...
        <el-card class="box-card">
            <div class="tableHead">
                <el-cascader :props="props" :options="options" v-model="dep"></el-cascader>
            ...
            <el-table :row-style="{ height: '55px' }" id="out-table" :data="list" style="width: 100%" height="660"
                :fit="true">
                <el-table-column fixed prop="id" label="科室id" min-width="90" align="center">
                </el-table-column>
                <el-table-column prop="name" label="名称" min-width="130" align="center">
                </el-table-column>
                <el-table-column prop="hosName" label="所属分院" min-width="180" align="center">
                </el-table-column>
                <el-table-column prop="hosId" label="分院id" min-width="90" align="center">
                </el-table-column>
                <el-table-column prop="depName" label="所属一级科室" min-width="140" align="center">
                </el-table-column>
                <el-table-column prop="depId" label="一级科室id" min-width="90" align="center">
                </el-table-column>
                <el-table-column prop="address" label="地址" min-width="130" align="center">
                </el-table-column>
            ...
            <el-dialog title="修改科室信息" :visible.sync="utw" width="500px">
                <el-form label-width="60">
                    ...
                    <el-form-item label="地址">
                        <el-input v-model="address" autocomplete="off" placeholder="请输入科室位置"></el-input>
                    </el-form-item>
                </el-form>
            ...
            <el-dialog title="添加二级科室" :visible.sync="addutw" width="500px">
                <el-form label-width="60">
                    <el-form-item label="所属科室">
                        <el-cascader :props="props" :options="options" v-model="adddep" style="width:360px;">
                        </el-cascader>
                    </el-form-item>
                    ...
                    <el-form-item label="楼层地址">
                        <el-input v-model="addaddress" autocomplete="off" placeholder="请输入科室对应楼层" style="width:360px;">
                        </el-input>
                    </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
...
export default {
    ...
    methods: {
        ...
        theadd() {
            if (this.addname == '' || this.adddep[1] == null) return this.$message.error('请填写完整信息')
            let data = {
                name: this.addname,
                depId: this.adddep[1],
        ...
        getInfo() {
            ...
            this.$http.get('simple/getDepTwo?paging=on&page=1&limit=12').then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data;
        ...
        post() {
            ...
            this.$http.post('admin/dep/moddepTwo', {
                depTwoId: this.id,
                ...
                if (res.data.code == 200) {
                    ...
                    this.list[this.index].address = this.address;
                } else {
                    this.$message.error(res.data.msg);
                }
            })
        ...
        del(index, id) {
            this.$confirm(`此操作将删除此科室, 是否继续?`, '提示', {
                ...
                this.$http.delete(`admin/dep/delDepTwo?id=${id}`).then(res => {
                    if (res.data.code == 200) {
                        this.$message.success(res.data.msg);
                        this.list.splice(index, 1);
                    } else if (res.data.code == 204) {
                        this.$prompt('请输入 “确认删除” 来强制删除所有内容', '警告，此科室下还有医生', {
                            ...
                            //强制删除
                            this.$http.delete(`admin/dep/delDepTwo?id=${id}&isStrict=strict`).then(res => {
                                if (res.data.code == 200) {
                                    this.$message.success(res.data.msg);
                                    this.list.splice(index, 1);
                                } else {
        ...
        onoroff($event, index) {
            ...
            this.$confirm(`此操作将${tw}此科室, 是否继续?`, '提示', {
                ...
                this.$http.put('admin/dep/switchDepTwo', data).then(res => {
                    if (res.data.code == 200) {
                        this.$message.success(res.data.msg);
                    } else {
        ...
        // 搜索功能
        searchBind() {
            console.log('this.dep', this.dep[1])
            if (this.dep[1] == null) return this.$message.error('请选择科室')
            const loading = this.$loading({
                lock: true,
                text: 'Loading',
                spinner: 'el-icon-loading',
            ...
            this.$http.get(`simple/getDepTwo?paging=on&depId=${this.dep[1]}&page=1&limit=15`).then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data
                    this.count = res.data.count
        ...
        // 全部导出，请求全部数据
        exportDataoz() {
            this.$http.get(`simple/getDepTwo?paging=on&page=1&limit=${this.count}`).then(res => {
                if (res.data.code == 200) {
        ...
        exportNewList(arr) {
            ...
            XLSX.utils.book_append_sheet(bookNew, workSheet, "二级科室列表");
            let name = "二级科室明细.xlsx";
            loading.close()
