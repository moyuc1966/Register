    ...
    <div class="main">
        ...
        <el-card class="box-card">
            ...
            <el-table :row-style="{ height: '55px' }" id="out-table" :data="list" style="width: 100%" height="660"
                ...
                <el-table-column prop="payType" label="支付方式" min-width="90" align="center">
                </el-table-column>
                <el-table-column label="支付状态" min-width="90" align="center">
                    <template slot-scope="scope">
                        <el-tag style="height:25px; line-height:25px;"
                            :type="scope.row.balance == '0' ? 'danger' : 'success'" disable-transitions>{{
                                    scope.row.balance == '0' ? "未完成" : "已完成"
                            }}
                        </el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="auantity" label="充值金额" min-width="100" align="center" :sortable="true">
                    <template slot-scope="scope">
                        ￥{{ scope.row.auantity.toFixed(2) }}
                    </template>
                </el-table-column>
                <el-table-column prop="balance" label="账户余额" min-width="110" align="center" :sortable="true">
                    <template slot-scope="scope">
                        {{ scope.row.balance == '0' ? "未完成" : "￥" + scope.row.balance.toFixed(2) }}
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            this.$http.get(`admin/recOrderList?page=${this.currentPage}&limit=${this.pageSize}`).then(res => {
        ...
        getInfo() {
            ...
            this.$http.get('admin/recOrderList?page=1&limit=12').then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data;
        ...
        searchBind() {
            ...
            this.$http.get(`admin/recOrderList?page=1&limit=15&${search}`).then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data
                    this.count = res.data.count
        ...
        yuan() {
            ...
            this.$http.get(`admin/recOrderList?page=1&limit=${this.pageSize}`).then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data
                    this.count = res.data.count
        ...
        // 全部导出，请求全部数据
        exportDataoz() {
            this.$http.get(`admin/recOrderList?page=1&limit=${this.count}`).then(res => {
                if (res.data.code == 200) {
        ...
        exportNewList(arr) {
            ...
            arr.forEach((item) => {
                let obj = {
                    ...
                    就诊卡号: item.card,
                    充值金额: item.auantity,
                    账户余额: item.balance == '0' ? '未完成' : item.balance,
                    支付类型: item.payType,
                    创建时间: item.time,
                    支付状态: item.balance == '0' ? '未完成' : '已完成'
                }
                tableData.push(obj);
            });
            ...
            XLSX.utils.book_append_sheet(bookNew, workSheet, "充值订单");
            let name = "充值订单列表明细.xlsx";
            loading.close()
