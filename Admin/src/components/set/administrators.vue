    ...
    <div class="main">
        ...
        <el-card class="box-card">
            ...
            <el-table :stripe="true" :row-style="{ height: '55px' }" id="out-table" :data="list" style="width: 100%"
                ...
                <el-table-column prop="id" label="管理员id" min-width="100" align="center">
                </el-table-column>
                <el-table-column fixed prop="name" label="管理员昵称" min-width="160" align="center">
                </el-table-column>
                <el-table-column prop="username" label="管理员账号" min-width="150" align="center">
                </el-table-column>
                <el-table-column prop="gender" label="管理员性别" min-width="100" align="center">
                    <template slot-scope="scope">
                        {{ scope.row.gender == 1 ? '女' : '男' }}
                    </template>
                </el-table-column>
                <el-table-column prop="email" label="绑定邮箱" min-width="180" align="center">
                    <template slot-scope="scope">
                        {{ scope.row.email == '' || scope.row.email == null ? '未绑定' : scope.row.email }}
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            this.$http.get(`admin/getAdministratorsList?page=${this.currentPage}&limit=${this.pageSize}`).then(res => {
        ...
        getInfo() {
            ...
            this.$http.get('admin/getAdministratorsList?page=1&limit=12').then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data;
        ...
        searchBind() {
            ...
            this.$http.get(`admin/administratorsSearch?page=1&limit=10&user=${this.search}`).then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data
                    this.count = res.data.count
        ...
        yuan() {
            ...
            this.$http.get(`admin/getAdministratorsList?page=1&limit=${this.pageSize}`).then(res => {
                loading.close()
                if (res.data.code == 200) {
                    this.list = res.data.data
                    this.count = res.data.count
        ...
        // 全部导出，请求全部数据
        exportDataoz() {
            this.$http.get(`admin/getAdministratorsList?page=1&limit=${this.count}`).then(res => {
                if (res.data.code == 200) {
        ...
        exportNewList(arr) {
            ...
            XLSX.utils.book_append_sheet(bookNew, workSheet, "管理员");
            let name = "管理员列表.xlsx";
            loading.close()
